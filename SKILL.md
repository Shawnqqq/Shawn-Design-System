...
description: "B2B 后台设计规范 + 代码规范。基于 Element Plus，确保页面风格统一、代码结构一致。"
---
...
### Step 2: 查找模板
查看 `templates/` 下最接近的模板。
- **相似度 > 70%**：直接使用该模板
- **相似度 < 70%**：列出候选模板，主动询问用户选择
→ 模板选择指南 → `specs/templates.md`

### Step 3: 选择组件
根据实际字段选择组件。→ 组件规范 → `specs/components.md`
...
- 圆角：2/4/8/12px 四级圆角
- 阴影：sm/md/lg/xl 四级阴影
→ 详细 Token → `specs/tokens.md`

### Step 5: 参考预览
查看 `preview/` 下对应的组件预览文件，了解组件用法和样式覆盖。

### Step 6: 生成代码
使用 Less 语法，遵循 `specs/code-standards.md` 中的 Vue 代码规范。
涉及 API/SQL 时，遵循 `specs/code-standards.md` 中对应章节。
...
| 文件 | 用途 | 何时读取 |
|------|------|---------|
| `DESIGN-SPEC.md` | 分层索引 + 速查表 | 需要快速查找 Token/组件/布局参数时 |
| `specs/tokens.md` | Token 详细定义 | 需要色阶、字体行高、间距场景等详细信息时 |
| `specs/components.md` | 组件规范 | 生成表格/表单/弹框等组件时 |
| `specs/templates.md` | 模板指南 | 选择页面模板或布局不确定时 |
| `specs/code-standards.md` | 代码规范 | 生成 Vue/API/SQL 代码时 |
| `templates/*.html` | 页面模板 | 参考布局思路 |
| `preview/*.html` | 组件预览 | 参考组件用法和样式覆盖 |
| `index.html` | 可视化导航 | 让用户预览效果 |
